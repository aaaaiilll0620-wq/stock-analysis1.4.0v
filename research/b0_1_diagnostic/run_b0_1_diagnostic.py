# -*- coding: utf-8 -*-
"""Frozen B0.1 RETROSPECTIVE DIAGNOSTIC REPLAY. Master v1.26 §12.8.
...
THIS IS NOT AN L2 RUN, AND IT IS STRUCTURALLY INCAPABLE OF BECOMING ONE.
  ...
  * it never writes under `artifacts/l2_run/` — its entire namespace is
    `artifacts/b0_1_diagnostic/`
  * it snapshots every Frozen B0 L2 artefact hash, `attempted_openings` and
    `effective_observation_count` BEFORE the first byte is written and asserts
    them unchanged AFTER the terminal record, so an accidental mutation is a
    failure rather than a footnote
  * it computes NO performance. NAV port_value is a state quantity the causal
    chain requires; cumulative wealth, CAGR, Sharpe_0rf and MDD are the
    evaluator's job and the evaluator refuses anything short of 141/141.

Strategy semantics come from the sealed B0.1 core and from nowhere else:
...
action happens behind `run_decision` / `core.b0_corporate_actions`.
    ...
    python research/b0_1_diagnostic/run_b0_1_diagnostic.py --authorization "<ref>"
"""
...
RUN_KIND = "B0_1_RETROSPECTIVE_DIAGNOSTIC"
...
DIAG_ROOT = os.path.join(REPO, "artifacts", "b0_1_diagnostic")
...
EXPECT = {
    "baseline_seal": "4d17505d3800b04c2f7cb867492038bd2d260b50b3b8fb9f99413a6da812c037",
    "spec_sha256": "6f452ea23406ac52a5547c32bb1285f4284dd07a83503ee865e88eb9b1145a15",
    "master_version": "1.26",
    "commit_prefix": "e708fdb7",
    "composed_market_state": "66640a7852aec84ada1e2ca5475998a05e41c0ae022fb60ac08a967e838ae1a4",
    "period1_full_input": "fdee3374f94681911bafb037df159d9970ff7ab8241256bb47c33945fd70d718",
...
def preflight(authorization):
    ...
    checks.append(_require(os.path.exists(seal_file), "B0.1 seal archived",
    ...
    foreign = [ln for ln in dirty
               if "research/b0_1_diagnostic" not in ln.replace("\\", "/")]
    ...
    checks.append(_require(len(NORMATIVE_MODULES) == 28, "28 normative modules",
                           str(len(NORMATIVE_MODULES))))
    bad = sorted(m for m in seal["normative_module_sha256"]
                 if measured.get(m) != seal["normative_module_sha256"][m])
    checks.append(_require(not bad, "28/28 normative modules match seal", str(bad)))
    ...
    checks.append(_require(composed == EXPECT["composed_market_state"],
                           "composed hash == 66640a78", composed))
    p1 = _load(P1_RECEIPT)["full_decision_input_sha256"]
    checks.append(_require(p1 == EXPECT["period1_full_input"],
                           "period-1 full input == fdee3374", p1))
...
def main():
    ...
    run_id = "B01DIAG-" + hashlib.sha256("|".join([
    ...
    provenance = {
        "record": "B0_1_DIAGNOSTIC_RUN_PROVENANCE",
        "run_id": run_id,
        "run_kind": RUN_KIND,
        "version": "B0.1",
        ...
        "b0_1_baseline_seal": EXPECT["baseline_seal"],
    ...
    def terminate(status, periods_done, detail, nav_rows=None):
        ...
        record = {
            "record": "B0_1_DIAGNOSTIC_TERMINAL_RESULT",
            "run_id": run_id,
            "run_kind": RUN_KIND,
            "evidence_class": EVIDENCE_CLASS,
            "confirmatory_l2": False,
            ...
            "performance_computed": False,
            "detail": detail,
            "frozen_b0_l2_snapshot_after": snap_after,
            "frozen_b0_l2_untouched": untouched,
            "b0_1_unmodified_during_replay": b01_untouched,
        }
        ...
        print("Frozen B0 L2 untouched: %s | B0.1 unmodified: %s"
    ...
    for i, period in enumerate(manifest, 1):
        ...
        try:
            ...
            return 3
        if i % 10 == 0:
            print("  %3d/141  %s  port_value=%.2f  positions=%d"
                  % (i, period["decision_month"], result.port_value,
