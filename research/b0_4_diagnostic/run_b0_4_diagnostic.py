# -*- coding: utf-8 -*-
"""Frozen B0.4 RETROSPECTIVE DIAGNOSTIC REPLAY. Master v1.29 §15.
...
action happens behind `run_decision` / `core.b0_corporate_actions`.
    ...
    python research/b0_4_diagnostic/run_b0_4_diagnostic.py --authorization "<ref>"
"""
...
RUN_KIND = "B0_4_RETROSPECTIVE_DIAGNOSTIC"
...
DIAG_ROOT = os.path.join(REPO, "artifacts", "b0_4_diagnostic")
...
EXPECT = {
    "baseline_seal": "09404edb7324eb813bc22498246a12e6189cfa752adb1bfd4de102951310aa49",
    "spec_sha256": "c10bafaca92386b040d6ad353595e4ea1b91c49ddf7e8248d775c2e858c3fd4d",
    "master_version": "1.29",
    "commit_prefix": "04edd975",
...
def preflight(authorization):
    ...
    checks.append(_require(os.path.exists(seal_file), "B0.4 seal archived",
    ...
    foreign = [ln for ln in dirty
               if "research/b0_4_diagnostic" not in ln.replace("\\", "/")]
    ...
    checks.append(_require(
        ...
        "141-state manifest matches the B0.4 seal", man_now))
    checks.append(_require(
    ...
    checks.append(_require(
        ...
        "CA ledger matches the B0.4 seal", ca_now))
...
def main():
    ...
    run_id = "B04DIAG-" + hashlib.sha256("|".join([
    ...
    provenance = {
        "record": "B0_4_DIAGNOSTIC_RUN_PROVENANCE",
        "run_id": run_id,
        "run_kind": RUN_KIND,
        "version": "B0.1",
        ...
        "b0_4_baseline_seal": EXPECT["baseline_seal"],
        ...
        "harness_path": "research/b0_4_diagnostic/run_b0_2_diagnostic.py",
    ...
    def terminate(status, periods_done, detail, nav_rows=None):
        ...
        record = {
            "record": "B0_4_DIAGNOSTIC_TERMINAL_RESULT",
            "run_id": run_id,
            "run_kind": RUN_KIND,
            "evidence_class": EVIDENCE_CLASS,
            "confirmatory_l2": False,
        ...
        "parent_b0_2_diagnostic_run": PARENT_B0_2_DIAGNOSTIC_RUN,
            ...
            "b0_4_unmodified_during_replay": b01_untouched,
