# -*- coding: utf-8 -*-
"""Frozen B0.2 RETROSPECTIVE DIAGNOSTIC REPLAY. Master v1.27 §13.
...
THIS IS NOT AN L2 RUN, AND IT IS STRUCTURALLY INCAPABLE OF BECOMING ONE.
  ...
  * it snapshots every Frozen B0 L2 artefact hash, every pinned Frozen B0.1
    diagnostic artefact, `attempted_openings` and `effective_observation_count`
    ...
    footnote. B0.2 replaces neither historical run.
...
action happens behind `run_decision` / `core.b0_corporate_actions`.
    ...
    python research/b0_2_diagnostic/run_b0_2_diagnostic.py --authorization "<ref>"
"""
...
RUN_KIND = "B0_2_RETROSPECTIVE_DIAGNOSTIC"
...
DIAG_ROOT = os.path.join(REPO, "artifacts", "b0_2_diagnostic")
...
EXPECT = {
    "baseline_seal": "904012383ca78a469127be6aa97a01d98fc783948ae100422531c7fff418bc71",
    "spec_sha256": "76bafdedfa4c55a3ebedc493771be2667eb2978ccc2cb468d901ce0a263614ea",
    "master_version": "1.27",
    "commit_prefix": "992ed1c1",
...
def preflight(authorization):
    ...
    checks.append(_require(os.path.exists(seal_file), "B0.2 seal archived",
    ...
    foreign = [ln for ln in dirty
               if "research/b0_2_diagnostic" not in ln.replace("\\", "/")]
    ...
    checks.append(_require(composed == EXPECT["composed_market_state"],
                           "141 composed hash == 66640a78", composed))
...
def main():
    ...
    run_id = "B02DIAG-" + hashlib.sha256("|".join([
    ...
    provenance = {
        "record": "B0_2_DIAGNOSTIC_RUN_PROVENANCE",
        "run_id": run_id,
        "run_kind": RUN_KIND,
        "version": "B0.1",
        ...
        "b0_2_baseline_seal": EXPECT["baseline_seal"],
        ...
        "harness_path": "research/b0_2_diagnostic/run_b0_2_diagnostic.py",
    ...
    def terminate(status, periods_done, detail, nav_rows=None):
        ...
        record = {
            "record": "B0_2_DIAGNOSTIC_TERMINAL_RESULT",
            "run_id": run_id,
            "run_kind": RUN_KIND,
            "evidence_class": EVIDENCE_CLASS,
            "confirmatory_l2": False,
            ...
            "b0_2_unmodified_during_replay": b01_untouched,
        ...
        print("historical lineage untouched: %s | B0.2 unmodified: %s"
