# -*- coding: utf-8 -*-
"""Frozen B0.5 RETROSPECTIVE DIAGNOSTIC REPLAY. Master v1.30 §16.
...
action happens behind `run_decision` / `core.b0_corporate_actions`.
    ...
    python research/b0_5_diagnostic/run_b0_4_diagnostic.py --authorization "<ref>"
"""
...
RUN_KIND = "B0_5_RETROSPECTIVE_DIAGNOSTIC"
...
DIAG_ROOT = os.path.join(REPO, "artifacts", "b0_5_diagnostic")
...
EXPECT = {
    "baseline_seal": "ab8a9a652074340b97269bde7f2894f46803f21a3e33a31b2532d8b283a4b9e1",
    "spec_sha256": "6c2f44853c4f3491165a6c1660a6fa8b024e2543e33f85e1aca2bbfb926bab3c",
    "master_version": "1.30",
    "commit_prefix": "7c4e099f",
...
def preflight(authorization):
    ...
    checks.append(_require(os.path.exists(seal_file), "B0.5 seal archived",
    ...
    foreign = [ln for ln in dirty
               if "research/b0_5_diagnostic" not in ln.replace("\\", "/")]
    ...
    checks.append(_require(
        ...
        "141-state manifest matches the B0.5 seal", man_now))
    checks.append(_require(
    ...
    checks.append(_require(
        ...
        "CA ledger matches the B0.5 seal", ca_now))
...
def main():
    ...
    run_id = "B05DIAG-" + hashlib.sha256("|".join([
    ...
    provenance = {
        "record": "B0_5_DIAGNOSTIC_RUN_PROVENANCE",
        "run_id": run_id,
        "run_kind": RUN_KIND,
        "version": "B0.1",
        ...
        "b0_5_baseline_seal": EXPECT["baseline_seal"],
        ...
        "harness_path": "research/b0_5_diagnostic/run_b0_2_diagnostic.py",
    ...
    def terminate(status, periods_done, detail, nav_rows=None):
        ...
        record = {
            "record": "B0_5_DIAGNOSTIC_TERMINAL_RESULT",
            "run_id": run_id,
            "run_kind": RUN_KIND,
            "evidence_class": EVIDENCE_CLASS,
            "confirmatory_l2": False,
        ...
        "parent_b0_4_diagnostic_run": PARENT_B0_4_DIAGNOSTIC_RUN,
            ...
            "b0_5_unmodified_during_replay": b01_untouched,
