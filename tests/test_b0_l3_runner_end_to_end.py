...
def run_dir(tmp_path_factory):
    ...
    write_aggregate(d, assemble_aggregate(
        run_dir=d, run_id=RUN, as_of=AS_OF,
...
@heavy
@sources
def test_decision_intent_builds_without_a_future_execution_fact(
        run_dir, opening_checkpoint):
    from core.b0_route import build_decision_intent

    built = R.build_period(
        run_dir, RUN, DECISION, _spans(), opening_checkpoint,
        opening_kind=ps.OPENING_GENESIS, c_ref=C_REF,
        expect_opening_period="2026-02", expect_opening_seq=1,
        synthetic_sources=True, decision_intent_only=True)

    assert built["execution_date"] is None
    assert built["assembled"]["price_span"][-1] == AS_OF
    assert built["assembled"]["price_span_endpoint_kind"] == \
        "AS_OF_DECISION_CUTOFF"
    assert all(r["execution_open"] is None for r in built["assembled"]["rows"])
    inp = built["decision_input"]
    assert not hasattr(inp, "execution_date")
    assert not hasattr(inp, "execution_prices")

    intent = build_decision_intent(inp, for_sealed_run=False)
    assert intent.decision_date == DECISION
    assert intent.as_of == AS_OF
    assert intent.targets.selected
    assert intent.stages[-1] == "order_intents"
...
def test_assemble_mode_writes_both_receipts_and_never_a_checkpoint(
    ...
    rc = R.main([
        ...
        "--genesis-cohort", R.SYNTHETIC_PARITY_COHORT,
