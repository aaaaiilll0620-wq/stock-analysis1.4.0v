...
> **規範性界線。** Frozen B0 sealed window 已交出之資訊 = 凍結規格於 decision date
> 2014-07-30（as_of 2014-07-30、execution 2014-08-01）所選出之 **20 檔標的名單
> 及其執行後狀態**；**未交出任何績效資訊**。
> 任何日後引用 Frozen B0 窗口之主張須揭露此範圍（含「決策已觀測、績效未觀測」之區分），
...
**機械強制（v1.37 落地，本版新增）。** 草稿 §5.1 實測揭露
`official Frozen B0 L2 replay permitted = false` 在 `core/` 中**沒有任何對應常數**，
它是文件宣告而非可執行閘門；`assert_rerun_admissible` 不知道自己已 moot。本版關閉此缺口：

```
core.b0_master_prereg.FROZEN_B0_LINEAGE                       "FROZEN_B0"
core.b0_master_prereg.LINEAGE_L2_REPLAY_PERMITTED             {FROZEN_B0: False}
core.b0_master_prereg.FROZEN_B0_REOPENING_UNREACHABLE_REASONS 兩個互相獨立的理由
core.b0_master_prereg.l2_replay_permitted(lineage)
core.b0_master_prereg.assert_l2_reopening_reachable(lineage)  → L2ReopeningUnreachable
```

`assert_reopening_admissible` 新增 `lineage` 參數，**預設為 `FROZEN_B0`**，
並於**任何其他檢查之前**先問可達性。預設值即是保護：**未具名者即為 Frozen B0 而被拒**，
只有明確具名之其他 lineage 方能到達 C-56 的機制。未知 lineage 回答「可達」是**依裁決**
而非疏漏 —— 本條範圍僅及 Frozen B0；欲關閉某 lineage，以具名方式加入該表。

三條 declaration conformance binding（`core/b0_declaration_conformance.py`）：

| key | kind | 所綁行為 |
|---|---|---|
| `frozen_b0_l2_replay_permitted` | IMPLEMENTATION_DERIVED | 宣告值即模組常數 |
| `frozen_b0_l2_reopening_is_unreachable` | BEHAVIORAL_CONFORMANCE | Frozen B0 在**任何**輸入組合下皆被拒（含**構造正確**之 repair、新 seal 與具名授權），且拒絕發生在其餘檢查之前；另一 lineage 仍到得了機制 |
...
回歸測試 `tests/test_b0_c72_observation_accounting.py` 另釘住本案事實：
`effective_observations()` 恆為 `('L2-af1b4d90c29b3b5f',)`，
且該列仍以原始標籤 F-CA-B 記錄。

> **既有測試之範圍調整（揭露）。** `assert_reopening_admissible` 之預設 lineage 為
> Frozen B0，故 `tests/test_b0_reopening_after_invalid.py`（5 處）與
> `tests/test_b0_condition_two.py`（1 處）中**測試機制本身**的呼叫改為具名
> `lineage="B1_LINEAGE_NOT_YET_OPENED"`。C-56 之機制與其斷言內容一字未改，
> 改變的只是它們所詢問的 lineage —— 若不調整，這些測試將全部被本條的可達性閘門攔下，
> C-56 的機制即無處可測。
...
- **落地之機械強制：** `LINEAGE_L2_REPLAY_PERMITTED`、`l2_replay_permitted`、
  `assert_l2_reopening_reachable`、`L2ReopeningUnreachable` 與
  `RECLASSIFIED_TERMINAL_ACCOUNTING_RULE`（`core/b0_master_prereg.py`）；
  `assert_reopening_admissible` 新增 `lineage` 參數（預設 `FROZEN_B0`）並於**任何其他
  檢查之前**先問可達性；三條 declaration binding
  （`frozen_b0_l2_replay_permitted`、`frozen_b0_l2_reopening_is_unreachable`、
  `l2_reclassification_does_not_reopen_accounting`）；
  新測試檔 `tests/test_b0_c72_observation_accounting.py`。
...
- ⚠ **既有測試之範圍調整（揭露）：** 因預設 lineage 為 Frozen B0，
  `tests/test_b0_reopening_after_invalid.py`（5 處）與
  `tests/test_b0_condition_two.py`（1 處）中測試 C-56 機制本身的呼叫改為具名
  `lineage="B1_LINEAGE_NOT_YET_OPENED"`。**斷言內容一字未改**，
  改變的只是所詢問的 lineage；若不調整，C-56 的機制將無處可測。
