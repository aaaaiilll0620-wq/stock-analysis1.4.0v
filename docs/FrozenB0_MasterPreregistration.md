...
**兩支腳本的 `--dry-run` 處置刻意不同（規範性）。**
被關閉的是**開封**，不是**稽核**：

| 入口 | 正式執行 | `--dry-run` |
|---|---|---|
| `b0_open_l2.py` | 拒絕 | **同樣拒絕** —— 它會印出一份「開封可行」的紀錄，而那個答案是錯的 |
| `b0_baseline_seal.py` | 拒絕（不得取得新 seal） | **允許** —— 組裝並驗證、**不寫 seal record、不寫 lineage ledger**，並須明白列印「Frozen B0 reopening UNREACHABLE」與「READ-ONLY audit」 |

sealer 的 `--dry-run` 是對凍結語料的唯讀一致性稽核，**正因窗口已關閉才更該保留**；
關閉一條開封路徑不構成刪除一個從未開啟任何東西的稽核之理由。
該模式仍受既有的乾淨工作區檢查拘束（與 C-72 無關），
且其輸出**不得**被引用為「已取得一次 Baseline Seal」。
...
| `l2_opening_entry_points_ask_the_gate` | BEHAVIORAL_CONFORMANCE | 以 **AST**（非字串比對）證明上列兩個入口確實 import 且 call 該 guard —— 註解或 docstring 裡的提及不算。綁的是「有沒有問」，處置（拒絕或唯讀放行）由各入口依上表決定 |
...
並以 **subprocess 實跑兩支腳本**：opener 之正式與 `--dry-run` 兩種輸入皆須非零離開、
訊息含 `9.6e-R5`，且 `artifacts/l2_run/` 之檔案樹**逐一比對前後相同**
（「什麼都沒建立」是量出來的，不是推論的）；
偽造 seal 那一則另證**拒絕發生在 seal 查找之前**——否則持有真 seal 者會發現邊界仍是開的。
sealer 則分兩則：正式執行須被拒且 `artifacts/baseline_seal/` 無任何寫入；
`--dry-run` 須**未被 C-72 閘門拒絕**、輸出含 UNREACHABLE 與 READ-ONLY audit、且同樣無寫入
（該則**刻意不斷言離開碼** —— 乾淨工作區檢查是既有規則，與本裁決無關）。
另一則直接對照兩支腳本的同一個旗標得到相反答案，把該不對稱釘成事實而非巧合。
...
- **落地之機械強制：** `REGISTERED_L2_LINEAGES`（窮舉、fail-closed）、
  ...
  `scripts/b0_baseline_seal.py`（repo identity snapshot 之前；正式執行拒絕，
  `--dry-run` **保留為唯讀稽核**，不寫 seal 亦不寫 ledger）；四條 declaration binding
