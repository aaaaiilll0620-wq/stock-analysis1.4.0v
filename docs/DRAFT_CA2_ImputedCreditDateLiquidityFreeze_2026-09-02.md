...
### §10.3 · 其餘 103 筆（含窗內全部 65 筆）是零星漏，形狀上可取數

窗內 65 筆散在 2014–2025、共 **33 檔證券**，各年缺失率 0.3%–5.0%。
**同年度絕大多數配股事件都有入帳日** ⇒ 該欄在來源端平常是存在的，
這些是個別的洞，不是結構性不可觀測。

否定證據一則：已採集的兩個官方端點
（TWSE `TWT49U` / `TWT49UDetail`、TPEx `exDailyQ`）欄位表中**沒有任何日期欄**，
全為價格與配股率（本 session 直接讀 `artifacts/stock_dividend_multiplier_audit/raw/`
的 payload 欄位清單確認）。
⇒ **既有採集回答不了這題，而能回答它的來源（除權公告本文 / 增資新股上市公告）
從未被嘗試。**「取不到」目前是未經檢驗的假設。

### §10.4 · 對 §5 判定的影響

預註冊 §3 的否證條款已經寫明「日後取得真實入帳日者一律以真實值為準」，
即**取數優於補值**是規則自己承認的。§10.3 顯示取數不只更好，而且形狀上可行。

⇒ 建議的落地順序修正為：

```
CA-1 closure
  → 65 筆入帳日取數（與 holder_side 90、capital_reduction 27 同一條採集線）
    → CA-2 適用範圍縮到「查證後仍取不到」的殘量
      → 殘量為 0 則 CA-2 不必落地，W-1 不必開例外
```

這件事的份量在於：**CA-2 是本專案史上第一個 `strategy semantics changed = true`。**
先取數能讓這個 true 適用於盡可能少的事件，甚至為零。

採集表已備妥：`_handoff/stock_dividend_credit_date_EXTRACTION_pass1_TOFILL.csv`（65 列），
指示書見同目錄 `GPT_EXTRACTION_INSTRUCTION_holder_side_terms.md` Part C。
