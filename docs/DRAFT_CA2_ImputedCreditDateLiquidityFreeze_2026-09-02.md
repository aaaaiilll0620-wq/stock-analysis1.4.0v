...
## §11 · ⚠ 追記二（2026-09-02 更晚）：窗內真正可能阻塞的是 25 筆，不是 65 筆

**本節改變的是 CA-2 的射程，不是它的論證。§4.2 與 §5 的數字據此重讀。**

### §11.1 · 40/65 的除權日晚於該證券自己的下市日

⟨M⟩ 以 repo 自有的 `data/b0/security_status.csv`（`status == "delisted"` 取最早
`effective_from`）逐列比對窗內 65 筆：

| 代號 | 下市日 | 事件數 | 事件日區間 |
|---|---|---:|---|
| 2315 | 2013-09-02 | 9 | 2015-06-17 … 2023-04-17 |
| 2470 | 2005-10-28 | 5 | 2018-07-10 … 2023-06-27 |
| 2319 | 2004-08-18 | 4 | 2021-12-08 … 2024-11-28 |
| 2416 | 2005-10-28 | 4 | 2020-07-01 … 2025-02-27 |
| 2452 | 2010-03-25 | 4 | 2016-06-29 … 2019-06-27 |
| 5480 | 2019-03-21 | 4 | 2022-06-09 … 2025-05-28 |
| 6008 | 2013-01-08 | 3 | 2018-06-21 … 2021-08-03 |
| 2325 | 2018-04-18 | 2 | 2024-04-25 … 2025-07-03 |
| 6251 | 2022-08-15 | 2 | 2024-03-26 … 2025-04-01 |
| 2403 / 4180 / 6178 | — | 各 1 | — |
| **合計** | **12 檔** | **40** | |

### §11.2 · 這 40 筆永遠不可能被暴露

⟨M⟩ `data/b0/price_panel.parquet`：上述 12 檔**沒有任何一檔**在其事件日當天或之後有價格列。
最後一筆價格都落在下市日當天或前一天；唯一的例外 `6251` 有 7 個交易日的尾巴
（末筆 2022-08-24），仍遠早於它 2024／2025 的兩筆事件。

推論鏈（依 §6.1.12 與 `ca_economic_interest_applies`）：

```
無價格  ⇒  不在可投資母體  ⇒  不可能被買進  ⇒  無 holding spell 覆蓋事件日
        ⇒  is_exposed = False  ⇒  §6.1.12 不觸發  ⇒  永遠不會中止 run
```

⚠ 該鏈條的前提是「持股僅能經由有價格的證券建立」。claim-side 暴露
（`ca_economic_interest_applies` 的第二個 domain）理論上可繞過價格，
但一筆 2005 年產生的 claim 不可能存活到 2020 年的事件日。此點為推理，非實測。

### §11.3 · 對數字的影響

```
                       預註冊 §4 記載      本節實測（扣除不可能暴露者）
CA-1 窗內                     312                        312   （0 筆下市後,不受影響）
CA-2 窗內                      65                     ★  25
CA-2 全庫                     160          108 在市 + 52 下市後
```

**CA-1 的數字完全正確。CA-2 的窗內射程被高估 2.6 倍。**
預註冊已凍結，依紀律**回報而不修改**。

### §11.4 · 這 40 筆是什麼

TEJ 對已被控股公司收購、私有化之後的子公司**繼續追蹤其資本額變動**。
2416 世平、2470 品佳（2005 併入大聯大）、2315 神達（2013 併入神達投控）、
2325 矽品（2018 併入日月光投控）皆屬此類。

事件本身是真的 —— 子公司確實辦了盈餘轉增資 —— 但**當時沒有任何公開持股人持有該證券**。
「配股入帳日」對它們不是查不到，是**不存在**。

⇒ 對這 40 筆，正確處置既不是 CA-2 的補值，也不是 `NOT_RECONSTRUCTIBLE`，
而更接近 CA-1 的形狀：**不是持股人事件**。本草案不提議該處置
（那是另一條規則，需要自己的預註冊），僅記錄此觀察。

### §11.5 · §10.3 留的那個開口已經關上

§10.3 揭露「MOPS 自由文本覆蓋率本 session 未能獨立複驗」。該複驗已由外部採集完成，
本 session 複驗其頭條主張（40/65 下市後）逐格吻合。採集方回報的其餘事實
（**繼承，非本 session 實測**）：

- 依日期的全市場公告表 `t05st02` 可取得**含已下市公司**的公告清單；
  已快取 3,186 個交易日、約 104 萬筆公告列。
- 但 `t05st02_detail` 對已停止公開發行的公司一律拒發本文
  （`該 XXXX 公開發行公司不繼續公開發行！`）⇒ **清單看得到，本文看不到。**
- 窗內 25 筆仍在市者中，11 筆查得公告、共讀入 108 篇本文：
  只載除權(息)交易日、最後過戶日、停止過戶起訖、除權(息)基準日與**現金股利發放日**，
  **無一載股票股利上市日**。
- 制度上該欄位存在（示範案例 `2332 友訊` 103/08/26 的重訊於「9.其他應敘明事項」
  載明「訂定103/11/6為增資新股發放上市及現金股利發放日」）——
  ⚠ **該示範案例不在本 65 筆之內**，它證明句型存在，不證明本母體查得到。

⇒ §10.3 的結論**方向不變但基礎更換**：不再是「公開來源沒有這個欄位」，
而是「**欄位在制度上存在，但本母體的發行人沒有填，且其中 40 筆的發行人在 MOPS 側已不存在**」。

### §11.6 · 對 §5 判定的影響

`strategy semantics changed = TRUE` 不變 —— 它仍然改變「股票何時可賣」。
改變的是**它影響幾個事件**：最多 25 筆，而非 65 筆。
自由參數仍為 1（`= 59`）。裁決的份量因此下降，但性質不變。

---

## §12 · ⚠ 追記三：59 天的可交換性假設被第一次檢驗，未通過

**本節提供的是 §5.2 第 1 點所稱「未經檢驗的假設」的第一批直接證據，方向對 CA-2 不利。**

### §12.1 · 觀測

使用者取得 `2315 神達電腦` 全部 9 筆缺值事件的實際股票發放日
（來源：中國信託法人信託歷年資料）。⟨M⟩ 本 session 計算落差：

| 除權日 | 發放日 | 落差(日曆天) | 在有值母體(n=8,109)的百分位 |
|---|---|---:|---:|
| 2015-06-17 | 2015-09-11 | 86 | 99.3% |
| 2016-07-07 | 2016-10-31 | 116 | 99.9% |
| 2017-07-04 | 2017-09-01 | 59 | 90.3% |
| 2018-05-24 | 2018-09-10 | 109 | 99.9% |
| 2019-05-09 | 2019-09-03 | 117 | 99.9% |
| 2020-05-15 | 2020-09-18 | 126 | 100.0% |
| 2021-04-15 | 2021-10-05 | 173 | 100.0% |
| 2022-04-14 | 2022-07-29 | 106 | 99.9% |
| 2023-04-17 | 2023-09-12 | 148 | 100.0% |

```
缺值樣本 n=9   min=59   中位數=116   max=173   平均=115.6   >59 者 8/9
有值母體 n=8,109      p50=44  p75=51  p90=59  p95=65  p99=81
```

### §12.2 · 這推翻了什麼

§3 選 p90 而非 p50 的論據是**方向性**：「填補缺值的方向必須讓策略吃虧而非佔便宜」。
該論據成立的前提是 59 天對缺值母體而言偏晚。

**觀測顯示相反：59 天是這 9 筆的最小值，中位數是它的兩倍。**
以 59 天補值，會讓投組平均提早約 57 天取得可賣股票 ——
**正是 §3 指名要避免的前視偏誤，只是幅度比 T+2 小。**

⇒ **`CA2_CREDIT_DATE_IMPUTATION_CALENDAR_DAYS = 59` 在方向上是錯的**，
不是幅度不夠保守，是符號反了。

### §12.3 · 這批證據的三個限制

1. **n=9，且全部同一發行人**（2315）。發行人層級的效果無法與母體層級分離。
2. **全部屬 §11 的「下市後私有子公司」族**，即不可能被 B0 持有的那 40 筆。
   對窗內真正有效的 25 筆是否適用，**未知**。
   合理的機制假說：私有子公司的配股沒有交易所時程壓力，撥發自然較慢；
   若成立，則 25 筆在市者的落差會比這 9 筆短。**該假說本身也未經檢驗。**
3. **來源為法人信託之歷史紀錄，屬 R4 的 `third_party_dataset`**
   ⇒ comparator-only，**不得決定任何 canonical 值**。
   本節僅作比較用途，未將任何一筆寫入 ledger 或任何 schema 欄。

### §12.4 · 對本草案的影響

不影響 §11 的射程結論（25 筆），也不影響 §5 的 `semantics changed = TRUE`。
影響的是**參數本身的可辯護性**：

- 若採 CA-2，59 這個值**不能沿用 §3 的方向性論證**，該論證已被觀測反駁。
- 重新選值需要缺值母體的實際落差分布，而目前只有 n=9、單一發行人、
  且屬不可能暴露的那一族。**這不足以支撐任何新的參數值。**
- ⇒ 現階段的誠實表述是：**CA-2 的參數目前沒有可辯護的取值**。
  這使選項空間往「不補值」的方向移動 —— 即 M-3／M-5 的
  sentinel 路線（owned but never tradable）在配股族也可能是更正確的形狀。
  **本草案不提議該變更**（那會改變已凍結的預註冊規則），僅記錄此指向。

### §12.5 · 這條線接下來要什麼

要判斷 59 是否可救，需要的是**窗內 25 筆在市事件**的實際發放日，
而不是更多下市子公司的。取得管道與本節相同（法人信託／股務代理歷史紀錄），
但其身分仍是 comparator-only：它可以**否證**一個補值參數，
不能**確立**一個 canonical 入帳日。
