...
## §10 · ⚠ 追記（2026-09-02 稍後）：160 筆不是同一個母體，且多數可能取得到

**本節在 §1–§9 寫定之後查得，結論足以改變 CA-2 的適用範圍，故追記於此而非改寫上文。**
...
### §10.2 · 2026 那 57 筆是匯出 vintage，不是資料缺口

該 57 筆的除權日為 `2026-05-14 ~ 2026-08-17`，而語料匯出截止即 **2026-08-17**
（來源目錄 `tej_exports/DataExport0806/配股相關2004-20260817`）。
入帳落差中位數 44 天 ⇒ **六月底之後除權的事件，在匯出當下尚未入帳，公告也尚未存在。**

⇒ 對這 57 筆補值，是為一批遲早會有真值的事件製造假值。
它們落在 141 期窗口之外，故不影響現行窗口，
但**任何前瞻 lineage（B1）都會踩到**。該族的正解是重新匯出，不是 CA-2。

### §10.3 · 其餘 103 筆（含窗內全部 65 筆）是 TEJ 的洞，但公開來源補不起來

**⚠ 本節於 2026-09-02 稍後改寫。原文主張「形狀上可取數」，
其後的取數嘗試對該主張的後半提供了反證，改寫如下；前半反而更成立。**

窗內 65 筆散在 2014–2025、共 **33 檔證券**，各年缺失率 0.3%–5.0%，
**同年度絕大多數配股事件都有入帳日** ⇒ 該欄在 TEJ 端平常是有值的，
這些是個別的洞，不是市場沒有產生過這個資訊。此點未變。

改寫的是「因此可取數」那一步。⟨M⟩ 本 session 逐一查證**四個**官方來源，
全部不含配股入帳日：

| 來源 | 內容 | 有入帳日 |
|---|---|---|
| TWSE `TWT49U` / `TWT49UDetail`、TPEx `exDailyQ` | 價格與配股率（讀 `artifacts/stock_dividend_multiplier_audit/raw/` 的 payload 欄位清單） | 無 |
| TWSE `rwd/zh/exRight/TWT48U` 除權除息預告表 | 除權息日期、無償配股率、現金增資配股率、認購價、現金股利、參考價 | 無 |
| TWSE OpenAPI `opendata/t187ap45_L` 上市公司股利分派情形 | 24 欄；日期僅「董事會（擬議）股利分派日」與「股東會日期」 | 無 |
| TWSE OpenAPI 全目錄（86 端點） | 無任何「增資新股上市」端點；帶「撥券日期」者僅 `announcement/publicForm`，屬 IPO 公開申購 | — |

外部抽取（Part C，65 列）另提供兩則**制度性**證據：
`6730 常廣` 的盈餘轉增資公告載新股數與除權基準日，
**並明載「發放日等事項授權處理」而未給實際日期**；
`0009A0 永豐金證券` 同一形狀。
⇒ 台灣的揭露制度公布比例與基準日，**把實際發放日授權董事會決定，
而該日期不進任何公開結構化資料**。

**由此得到的推論，方向與原文相反：**

1. 既然公開結構化來源沒有這個欄位，TEJ 那 8,109 筆的入帳日**不可能**來自這四個端點。
   它另有管道（股務代理／集保饋送／人工蒐集），該管道對 8,109 筆有效、對這 65 筆失效。
2. ⇒ 取數路線應指向 **TEJ 本身**（換表、重新匯出、或查詢缺值原因），
   **不是**繼續掃公開來源。
3. ⇒ 在 TEJ 路線有結果之前，**「公開來源取不到」已由四項獨立查證支持**，
   不再是未經檢驗的假設。

⚠ 仍未窮盡者：MOPS 自由文本重大訊息全文。Part C 宣稱查過三個表號
（t05st01 / t59sb09 / t146sb10）而 65 筆中僅 3 筆找到對應的盈餘轉增資公告，
該覆蓋率本 session **未能獨立複驗**。此為本節唯一的開口。

### §10.4 · 對 §5 判定的影響

預註冊 §3 的否證條款已經寫明「日後取得真實入帳日者一律以真實值為準」，
即**取數優於補值**是規則自己承認的。這一點不因 §10.3 的改寫而動搖：
若取得到，真實值仍然壓過推估值。

改變的是**取得到的機率**。原文據以主張「先取數」的理由是「形狀上可行」，
而 §10.3 的四項查證顯示公開來源沒有這個欄位。⟨M⟩ 一輪實際採集
（Part C，65 列）的結果是 **65/65 `BLOCKED`、0 筆查實**。

⇒ 落地順序修正為：

```
CA-1 closure
  → TEJ 路線（換表／重新匯出／查詢缺值原因）—— 唯一尚未嘗試且有理由成功的路
    → 成功則 CA-2 適用範圍縮到殘量;殘量為 0 則 CA-2 不必落地
    → 失敗或無法進行，則 CA-2 由「備案」升為「必要」,
      且「取不到」屆時是查證過公開來源與廠商來源兩層的結論
```

這件事的份量在於：**CA-2 是本專案史上第一個 `strategy semantics changed = true`。**
它適用於幾個事件，取決於上面那條路的結果 —— 但已經不再有「先取數就能歸零」的預期。

⚠ **該族與減資同樣沒有凍結來源政策授權落地**，採集本身不改動任何東西。
Part C 的空手而回**不消耗**任何額度，也不構成對 CA-2 的背書 ——
它只是把「取不到」從假設變成一項有證據的主張。

### §10.5 · 對 §5.2 第 1 點的補強

§5.2 已揭露「65 筆缺值事件未必與 8,109 筆有值事件同分布」是未經檢驗的假設。
§10.1 使該疑慮更具體：**缺失率自 0.3% 單調升至 5.0%，缺失顯然不是隨機的。**
以有值事件的 p90 估缺值事件，可交換性假設比 §5.2 原本的措辭更弱。
