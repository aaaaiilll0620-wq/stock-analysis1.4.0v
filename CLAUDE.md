...
| 層 | 模型 | 做什麼 |
|---|---|---|
| 指揮官 | Opus 5 / high | 裁定。**只採信自己查過的事實** |
| 反證者 `b0-refuter` | Sonnet 5 / high | 攻擊一個具體主張 → 反例 |
| 掃描員 `b0-doc-sweeper` | Haiku 4.5 | 大量讀取 → 指標 |
...
簽字前      任何要進裁決/closure/commit/回報的發現 → 先丟給 b0-refuter 攻一輪
```
...
2. **來源必須標記。** 子代理的產出是「繼承自其他來源」，不是「本 session 驗證過」。
   ...
   ⚠ 子代理換更強的模型**會讓這條更危險不是更安全**：弱模型的錯答語氣心虛容易抓，
   強模型的錯答自信、完整、有引用，指揮官更可能直接採信。

3. **反證可以外包，驗證不行**——差別在回報的不對稱性。
   「推不翻」什麼都沒證明，所以不會產生假的背書；「推翻了」交回的是一個反例，
   而反例是指標。所以最強的子代理放在**反證**這一格，不是「驗證」那一格。
   `b0-refuter` 永遠不准回報「這個主張成立」。
