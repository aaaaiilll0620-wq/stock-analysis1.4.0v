...
產出 digest_{date}.md —— 新進 composite 前 N、掉出前 N、連續在榜天數、來源臂、regime 旗。
...
def md_table(df: pd.DataFrame, streaks: dict, cap: int = 10**9, extra_cols=()) -> str:
    lines = ["| 代號 | 名稱 | 產業 | 收盤 | composite | 來源臂 | 連續在榜 |" +
    ...
    for sid, r in df.iterrows():
        ...
        lines.append(f"| {sid} | {r.get('name', '')} | {r.get('industry', '')} "
                     f"| {r.get('close', float('nan')):.1f} | {r['composite']:.1f} "
...
def main():
    ...
    today = load(today_f).sort_values("composite", ascending=False)
    ...
    prev_top = set()
    if len(files) >= 2:
        prev = load(files[-2]).sort_values("composite", ascending=False)
    ...
    stayers = (today[[sid in streaks and streaks[sid] >= args.streak_min for sid in today.index]]
               ...
               .sort_values(["_s", "composite"], ascending=False).head(30))
    ...
    parts = [
        ...
        f"",
        f"市場 regime:{regime}",
        f"shortlist {len(today)} 檔｜composite 前 {args.top}:新進 {len(new_in)} 檔、掉出 {len(dropped)} 檔",
        f"",
        f"## 🆕 新進 composite 前 {args.top}",
        md_table(new_in, streaks, cap=len(hist_sets)) if len(new_in) else "(無)",
        f"",
        f"## 🔥 連續在榜 ≥{args.streak_min} 天 (依天數排,前 30)",
        md_table(stayers.drop(columns=['_s']), streaks, cap=len(hist_sets)) if len(stayers) else
